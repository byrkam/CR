...
SSID="poutsa"
PSK="labwpa3pass"
WPA_CONF="configs/wpa_supplicant_wlan3.conf"
log() { echo "[STA:wlan3] $1"; }
wait_for_iface() {
  for i in $(seq 1 80); do ip link show "wlan3" >/dev/null 2>&1 && return 0; sleep 0.25; done
  echo "iface wlan3 not present in ns"; exit 1
}
wait_for_iface
