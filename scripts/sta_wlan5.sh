...
SSID="lab-wpa2"
PSK="labpassword"
WPA_CONF="configs/wpa_supplicant_wlan5.conf"
log() { echo "[STA:wlan5] $1"; }
wait_for_iface() {
  for i in $(seq 1 80); do ip link show "wlan5" >/dev/null 2>&1 && return 0; sleep 0.25; done
  echo "iface wlan5 not present in ns"; exit 1
}
wait_for_iface
